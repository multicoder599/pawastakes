...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/pawastakes';
...
// ==========================================
// TELEGRAM HELPERS (DUAL BOT SETUP)
// ==========================================

const sendTelegramMessage = async (message, target = 'deposit') => {
    let token, chatId;
    if (target === 'bet') {
        token = process.env.TELEGRAM_BET_BOT_TOKEN;
        chatId = process.env.TELEGRAM_BET_CHAT_ID;
    } else {
        token = process.env.TELEGRAM_DEPOSIT_BOT_TOKEN;
        chatId = process.env.TELEGRAM_DEPOSIT_CHAT_ID;
    }
...
const normalizePhone = (phone) => {
    ...
    if (digitsOnly.startsWith('254')) {
        if (digitsOnly.charAt(3) === '0') {
            digitsOnly = '254' + digitsOnly.slice(4);
        }
        return '+' + digitsOnly;
    }
    ...
    if (/^[71]/.test(digitsOnly) && digitsOnly.length === 9) {
    ...
    try {
        ...
        if (formattedPhone.length !== 12) {
            return res.status(400).json({
                success: false,
                message: 'Invalid phone number format. Use 07XXXXXXXX, 7XXXXXXXX, or +254XXXXXXXXX.'
            });
        }
        ...
        const payload = {
            api_key:      process.env.MEGAPAY_API_KEY  || 'MGPY5RgqieBq',
            email:        process.env.MEGAPAY_EMAIL    || 'streetmaster878@gmail.com',
            amount:       amount,
            msisdn:       formattedPhone,
            callback_url: `${process.env.APP_URL || 'https://api.pawastakes.com'}/api/megapay/webhook`,
            description:  'Pawastakes Deposit',
            reference:    reference
        };
...
app.post('/api/auth/register', rateLimit({ windowMs: 60 * 60 * 1000, max: 15 }), async (req, res) => {
    try {
        // 🔥 CRITICAL: Defensive check for malformed body
        ...
        const isKenyan = normalizedPhone.startsWith('+254');
        if (isKenyan && !isValidKenyanPhone(normalizedPhone)) {
            return res.status(400).json({
        ...
        const existingUser = await User.findOne({
            $or: [
                ...
                { email: { $regex: new RegExp('^' + escapeRegExp(trimmedEmail) + '$', 'i') } },
        ...
        if (existingUser) {
            ...
            if (existingUser.phone === normalizedPhone) {
                return res.status(400).json({ error: "Phone number is registered." });
            }
        }

        const currency = isKenyan ? 'KES' : 'USD';
        const countryCode = isKenyan ? 'KE' : 'US';
        const timezone = getTimezoneFromCountry(countryCode, normalizedPhone);
        ...
        // 🔥 CRITICAL: Notification & Telegram are non-blocking.
        // If they fail, the user is already created — we MUST NOT return 500.
        try {
            ...
            console.error('❌ Notification save failed (non-critical):', notifErr.message);
        }
        ...
        console.log(`✅ Registration success: ${trimmedUsername} | ${normalizedPhone}`);
    ...
    try {
        ...
        const user = await User.findOne({
            $or: [
                ...
                { username: { $regex: new RegExp('^' + escapeRegExp(trimmedIdentifier) + '$', 'i') } },
                { phone: phoneQuery },
...
app.post('/api/wallet/deposit/manual', async (req, res) => {
    try {
        ...
        sendTelegramMessage(`⏳ <b>MANUAL DEPOSIT</b>\n👤 User: ${user.username}\n💳 Method: ${method}\n💰 Amount: ${amount} ${currency}`, 'deposit');
        res.status(200).json({ message: "Deposit requested. 100% bonus applies for KES deposits of 500 and above.", balance: user.balance });
...
app.post('/api/bets/save-code', async (req, res) => {
    try {
        ...
        await BookingSlip.findOneAndUpdate({ code: code.toUpperCase() }, { code: code.toUpperCase(), legs, stake, totalOdds, potentialReturn, currency }, { upsert: true, returnDocument: 'after' });
    ...
    try {
        ...
        if (action === 'approve') {
            ...
            if (txn.type === 'Deposit') {
                ...
                let bonus = 0;
                if (user.currency === 'KES' && txn.amount >= 500) bonus = txn.amount;
                user.balance += txn.amount + bonus; await user.save();
                await new Notification({ userId: user._id, title: "Deposit Approved", message: `Your deposit of ${txn.amount} ${txn.currency} was approved.${bonus ? ' Bonus: ' + bonus + ' ' + txn.currency : ''}` }).save();
            }
            ...
            for (let leg of bet.legs) {
                ...
                if (resultObj) {
                    ...
                    if (pickStr.match(/^\d+-\d+$/)) {
                        ...
                        else if ((pickStr === 'X' || selStr === 'DRAW' || selStr.includes('DRAW')) && hG === aG) isWin = true;
